// ─── Mahjong AI Strategies — 12 strategies with named theory models ───────────
import { tileKey, sortHand, checkWin, getTenpaiTiles, calcShanten, calcFan, SUITS, WIND_ZH, DRAGON_ZH } from '../game/tiles.js'
...
function scoredDiscard(hand, discards, melds, scoreFn) {
  ...
  if (!playable.length) return hand[0] || null
  let best=playable[0], bestScore=-Infinity
  for (const tile of playable) {
    const rem   = playable.filter(t => t.id !== tile.id)
...
export const AI_STRATEGIES = {
  nash: {
    ...
    wantRon:(tile,hand,melds)=>{const test=sortHand([...hand,tile]);if(!checkWin(test,melds))return false;const{fan}=calcFan(test,melds,false,0,0);return fan>=3},
  },
  dragon: {
  ...
  tortoise: {
    ...
    wantRon:(tile,hand,melds)=>{const test=sortHand([...hand,tile]);if(!checkWin(test,melds))return false;const{fan}=calcFan(test,melds,false,0,0);return fan>=5},
  },
  tripletHunter: {
    name:'對對糊專家', fullName:'Triplet Hunter', emoji:'🎯', color:'#e17055', theory:'Pattern Specialisation',
    desc:'專攻對對糊。優先保留對子和刻子，寧願叫慢不做順子。',
    discard:(h,d,m)=>scoredDiscard(h,d,m,(t,r)=>{
      const rc={}; for(const x of r){const k=tileKey(x);rc[k]=(rc[k]||0)+1}
      return -calcShanten(r,m)*8 + Object.values(rc).filter(v=>v>=2).length*5 + Object.values(rc).filter(v=>v>=3).length*10
    }),
    wantRon:(tile,hand,melds)=>{const test=sortHand([...hand,tile]);if(!checkWin(test,melds))return false;const{fan,label}=calcFan(test,melds,false,0,0);return fan>=3&&(label[0]==='對對糊'||fan>=5)},
  },
  sevenPairs: {
    ...
    discard:(h,d,m)=>{
      ...
      return scoredDiscard(h,d,m,(t,r)=>{
        const rc={}; for(const x of r){const k=tileKey(x);rc[k]=(rc[k]||0)+1}
        return -(6-Object.values(rc).filter(v=>v>=2).length)*12 + ((rc[tileKey(t)]||0)===0?5:0)
      })
    },
    wantRon:(tile,hand,melds)=>{const test=sortHand([...hand,tile]);if(!checkWin(test,melds))return false;const{fan}=calcFan(test,melds,false,0,0);return fan>=3},
  },
  honourMaster: {
    ...
    discard:(h,d,m)=>scoredDiscard(h,d,m,(t,r)=>{
      const rc={}; for(const x of r){const k=tileKey(x);rc[k]=(rc[k]||0)+1}
      const isH=t.suit==='winds'||t.suit==='dragons'
      const simple=SUITS.includes(t.suit)&&t.value>=2&&t.value<=8
      return -calcShanten(r,m)*10 + (simple&&(rc[tileKey(t)]||0)<=1?5:0) + (isH&&(rc[tileKey(t)]||0)>=2?-8:0)
    }),
    wantRon:(tile,hand,melds)=>{const test=sortHand([...hand,tile]);if(!checkWin(test,melds))return false;const{fan}=calcFan(test,melds,false,0,0);return fan>=3},
  },
  monteCarlo: {
    ...
    discard:(h,d,m)=>{
      const nf=h.filter(t=>!t.isFlower)
      if(!nf.length) return h[0]||null
      let best=nf[0], bestS=-Infinity
      for(const t of nf){const r=nf.filter(x=>x.id!==t.id);const s=-calcShanten(r,m)*10+Math.random()*3;if(s>bestS){bestS=s;best=t}}
      return best
    },
    wantRon:(tile,hand,melds)=>{const test=sortHand([...hand,tile]);if(!checkWin(test,melds))return false;const{fan}=calcFan(test,melds,false,0,0);return fan>=3},
  },
  opportunist: {
    ...
    discard:(h,d,m)=>{
      const urgency=d.flat().length>40?0.8:0.3
  ...
  bigHand: {
    ...
    discard:(h,d,m)=>scoredDiscard(h,d,m,(t,r)=>{
      ...
      return -calcShanten(r,m)*10 + Object.values(rc).filter(v=>v>=3).length*7 + htrip*6
    }),
    wantRon:(tile,hand,melds)=>{const test=sortHand([...hand,tile]);if(!checkWin(test,melds))return false;const{fan}=calcFan(test,melds,false,0,0);return fan>=5},
  },
  safeRunner: {
  ...
  chaos: {
    ...
    discard:(h,d,m)=>{
      const nf=h.filter(t=>!t.isFlower)
      if(!nf.length) return h[0]||null
      return Math.random()<0.3?nf[Math.floor(Math.random()*nf.length)]:scoredDiscard(h,d,m,(t,r)=>-calcShanten(r,m)*10)
    },
    wantRon:(tile,hand,melds)=>{const test=sortHand([...hand,tile]);if(!checkWin(test,melds))return false;const{fan}=calcFan(test,melds,false,0,0);return fan>=3},
  },
}
...
export function aiDiscard(stratKey, hand, allDiscards, melds=[]) {
  ...
  const result = s.discard(hand, allDiscards, melds)
  // Fallback: if strategy returns null, discard last non-flower tile
  if (!result) {
    const nf = hand.filter(t=>!t.isFlower)
    return nf[nf.length-1] || hand[hand.length-1] || null
  }
  return result
}
...
export function aiWantsRon(stratKey, tile, hand, melds=[]) {
  const s = AI_STRATEGIES[stratKey] || AI_STRATEGIES.nash
  try { return s.wantRon(tile, hand, melds) } catch { return false }
}
